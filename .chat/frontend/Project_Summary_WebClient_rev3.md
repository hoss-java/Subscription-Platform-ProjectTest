...
| 13 | `headerInstance.update()` | Updates header to show username + logout button |
...
| 18 | Logout | `authService.logout()` clears tokens, `headerInstance.update()` shows login link |
...
- Updated `index.html` with `id="user-info-container"` in header
- Created `pages/header.html` (shared header component)

### ⏳ Next Steps:
1. Create `HeaderComponent` class in components directory
2. Update `config/scripts.json` to include header component
3. Add header CSS to both theme directories
4. Update `config/styles.json` to include header CSS
5. Implement actual proxy connection in `apiClient.js`
6. Add 401 response interceptor with token refresh
7. Test login flow with mocked backend
8. Create unit tests for `authService` and `authGuard`
9. Add error handling UI components (toast notifications)
...
2. Create corresponding JS file in `pages/` subdirectory
